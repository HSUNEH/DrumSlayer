...
def generate_midi(args):
    ...
    for b in range(3):
        ...
        for _ in tqdm(range(midi_number)):
            ...
            # write to file
            output_dir = f'generated_data/drum_data_{data_type}/generated_midi/{midi_name}'
            os.makedirs(output_dir, exist_ok=True)
            output_file = os.path.join(output_dir, f'{midi_name}_{_}.midi')
            mido_obj.dump(output_file)

            # write to midi file
            output_dir = f'generated_data/drum_data_{data_type}/generated_midi_numpy/{midi_name}_numpy'
            os.makedirs(output_dir, exist_ok=True)
