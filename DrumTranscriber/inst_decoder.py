...
class InstDecoder(nn.Module):
    def __init__(self, config):
        ...
        self.audio_embedding_layer = nn.ModuleList([nn.Embedding(config.audio_rep_dim, config.dec_d_model) for _ in range(9)]) 
    ...
    def forward(self, x, y=None, strategy="greedy", sample_arg=None):
        ...
        # y: [batch_size, seq_len, 10]                                      torch.Size([4, 1471, 10])   target_midi_audio_token
            ...
            embedding = self.positional_encoder(y_tok_embedding) # [batch_size, seq_len, d_model] -> [batch_size, seq_len, d_model] :[4, 1471, 768]
